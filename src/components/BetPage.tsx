...
import { supabase, type Profile, type MatchOdds } from "../lib/supabase";
...
export default function BetPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("match_odds").select("*").then(({ data }) => {
  ...
  const od = (m: MatchInfo) => odds[m.id] || { home_win: 2.50, draw: 3.20, away_win: 2.50 };
