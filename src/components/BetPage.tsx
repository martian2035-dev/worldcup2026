...
import {
  getProfile, createProfile, getBets, addBet,
...
export default function BetPage() {
  ...
  const [showAuth, setShowAuth] = useState(!getProfile());
  ...
  const handleBet = (match: MatchInfo, betType: BetType, oddsValue: number) => {
    ...
    if (!fresh || fresh.beans < 10) { setMsg("余额不足，至少需要 10 豆"); return; }
  ...
  // 注册/登录
  if (showAuth) return (
    <div style={{ maxWidth: 380, margin: "40px auto", background: "var(--color-bg, #0a1628)", borderRadius: 16, padding: 28, border: "1px solid rgba(255,255,255,0.08)", textAlign: "center" }}>
      ...
      <p style={{ color: "var(--color-text-muted)", fontSize: 10, marginTop: 14 }}>数据仅保存在浏览器本地，不涉及任何金钱</p>
    </div>
  );
  ...
  return (
    <div>
      {msg && <div style={{ textAlign: "center", padding: 8, color: msg.includes("✅") ? "var(--color-positive)" : "#E53935", fontSize: 13 }}>{msg}</div>}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button onClick={() => { clearLocalData(); setProfile(null); setShowAuth(true); }} style={{ background: "none", border: "none", color: "var(--color-text-muted)", cursor: "pointer", fontSize: 10 }}>重置</button>
...
function updateLocalBeans(delta: number): ReturnType<typeof getProfile> {
...
function clearLocalData(): void {
  ...
  localStorage.removeItem("wc2026_bets");
}
